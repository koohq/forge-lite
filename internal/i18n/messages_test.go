...
func TestHubMenu_PartnerSync_JA(t *testing.T) {
	...
	tests := []struct {
		...
		{"3: AttachOrb", msg.HubMenu3AttachOrb(th.InstallVerb), "3: 連携を組む (パッシブ効果の装着)"},
		{"4: Disassemble", msg.HubMenu4Disassemble(th.OrbLabel), "4: 連携スキルを分解 (素材への還元)"},
		{"5: Synthesize", msg.HubMenu5Synthesize(th.OrbLabel), "5: 連携スキルを合成 (上位性能への強化)"},
		{"6: EnhanceHp", msg.HubMenu6EnhanceHp(th.EnhanceHPVerb, th.HPLabel), "6: 継戦力を鍛える (継戦力 +10)"},
		{"7: Settings", msg.HubMenu7Settings, "7: 設定 / Settings"},
		{"0: Exit", msg.HubMenu0Exit, "0: ゲーム終了"},
	}
...
func TestHubMenu_PartnerSync_EN(t *testing.T) {
	...
	tests := []struct {
		...
		{"1: Dungeon", msg.HubMenu1Dungeon, "1: Embark to Dungeon (Start Run)"},
		{"2: Enhance", msg.HubMenu2Enhance(th.EnhanceVerb, th.StatLabel), "2: Train with Iris (Upgrade Synergy)"},
		{"3: AttachOrb", msg.HubMenu3AttachOrb(th.InstallVerb), "3: Form Tactics (Equip Passives)"},
		{"4: Disassemble", msg.HubMenu4Disassemble(th.OrbLabel), "4: Dismantle Link Skill (Convert to Materials)"},
		{"5: Synthesize", msg.HubMenu5Synthesize(th.OrbLabel), "5: Synthesize Link Skill (Upgrade to Plus)"},
...
func TestHubStats_DynamicAbyss(t *testing.T) {
	...
	expectedJA := "最大体力: HP 100 | 事象の地平線 最高到達: B42F"
	if jaRes != expectedJA {
